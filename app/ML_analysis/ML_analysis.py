...
class ML_run:
    ...
    def ML(self):
        ...
        x_train = train_df.drop(columns=['RN_INDI', 'case', 'STD_YYYY'])

        y_test = test_df[['case']]
        x_test = test_df.drop(columns=['RN_INDI', 'case', 'STD_YYYY'])
        ...
        grid_search = GridSearchCV(estimator=model, param_grid=params_grid, scoring='roc_auc',
                                   cv=5)  # auroc를 기준으로 모델에서 최적의 parameter를 찾습니다
        ...
        features = pd.DataFrame(model.feature_importances_.reshape(-1, 1), index=x_train.columns).sort_values(0,
                                                                                                              ascending=False)
        ...
        risk_case = target_date.case.sum()
        ...
        return results  # 이 값이 AnalysisResultML에 들어갑니다. 순서대로 fi_ratio, injected_case, risk_case 입니다.
